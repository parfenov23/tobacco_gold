...
var addProductItemToSale = function(id){
  ajaxApi("get", "/api/product_items/" + id, {}, function(result){
    ...
    if (!find_item.length){
      ...
      find_item.find("[data-type='id']").attr("name", "sales[][item_id]").val(id);
      find_item.find("[data-type='price']").attr("name", "sales[][price]").val(price);
      find_item.find("[data-type='count']").attr("name", "sales[][count]");
    ...
    price = find_item.find("input[name='sales[][price]']").val();
    var count_block = find_item.find("input[name='sales[][count]']");
...
var loadProductItems = function(){
  var btn = $(this);
  var id = btn.data("id");
  $.get( "/admin/sales/load_content_product_items?id=" + id + "&type_partial=new", function(data) {
...
var addProductItemWhenBarcode = function(barcode){
  ajaxApi("get", "/api/product_items/get_search", {barcode: barcode, type: "present"}, function(result){
    if(result.ids.length){
      if(result.ids.length > 1){
        $("input.search_product_items").val(barcode);
        sales_load_content_product_items(barcode);
      }else{
...
var sales_load_content_product_items = function(value){
  $.get( "/admin/sales/load_content_product_items?type_partial=new" + "&search=" + value, function(data) {
    $(".leftContent .pasteBlock a").remove();
    $(".content_sale .leftContent .pasteBlock").append(data);
    $(".leftContent .allProducts").fadeOut('slow', function() {
      $(".leftContent .pasteBlock").show();
...
$(document).ready(function(){
  ...
  $(document).on('keyup', '.search_product_items', function(){
    ...
    var input = $(this);
    if(input.val().length > 0){
      timout_search_product_items = setTimeout(sales_load_content_product_items(input.val()), 300);
    }else{
