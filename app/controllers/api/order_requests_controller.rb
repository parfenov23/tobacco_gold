module Api
  class OrderRequestsController < ApiController
    ...
    def create
      ...
      address = "ул.#{params_r[:address][:street]} д.#{params_r[:address][:house]} кв.#{params_r[:address][:room]}, подьезд: #{params_r[:address][:porch]}, этаж: #{params_r[:address][:floor]}, домофон: #{params_r[:address][:domofon]}"
