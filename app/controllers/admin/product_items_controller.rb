module Admin
  class ProductItemsController < AdminController

    def index
      @product_items = find_product.product_items
    end

    def new
      @product_item = ProductItem.new
    end

    def create
      ProductItem.create(params_product_item)
      redirect_to_index
    end

    def edit
      @product_item = find_product_item
    end

    def update
      find_product_item.update(params_product_item)
      redirect_to_index
    end

    def remove
      product_id = find_product_item.product_id
      find_product_item.destroy
      redirect_to '/admin/product_items?product_id=' + product_id.to_s
    end

    private

    def find_product
      Product.find(params[:product_id])
    end

    def find_product_item
      ProductItem.find(params[:id])
    end

    def redirect_to_index
      product_id = params_product_item[:product_id].present? ? params_product_item[:product_id] : find_product_item.product_id
      redirect_to '/admin/product_items?product_id=' + product_id
    end

    def params_product_item
      params.require(:product_item).permit(:title, :product_id, :count, :image_url, :description, :in_stock, :top, :barcode).compact rescue {}
    end
  end
end